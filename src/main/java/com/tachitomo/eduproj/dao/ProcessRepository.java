...
public class ProcessRepository implements ProcessMapper {
    ...
    public List<Role> findAll(){
    	return sqlSession.selectList("com.tachitomo.eduproj.repository.ProcessMapper.findAll");
    }   
    ...
    public Role findById(Long id) {
        return sqlSession.selectOne("com.tachitomo.eduproj.repository.ProcessMapper.findById", id);
    }	
    ...
    public void insert(ProcessAddRequest processRequest) {
    	int insertCount = sqlSession.insert("com.tachitomo.eduproj.repository.ProcessMapper.insert", processRequest);
        if (insertCount == 0) {
    ...
    public void update(ProcessUpdateRequest processUpdateRequest) {
    	int updateCount = sqlSession.update("com.tachitomo.eduproj.repository.ProcessMapper.update", processUpdateRequest);
        if (updateCount == 0) {
            throw new OptimisticLockingFailureException("楽観ロックエラー");
        }
    }    
    ...
    public void delete(Long id) {
    	int deleteCount =  sqlSession.delete("com.tachitomo.eduproj.repository.ProcessMapper.delete", id);
        if (deleteCount == 0) {
            throw new OptimisticLockingFailureException("楽観ロックエラー");
        }    	
    }      
